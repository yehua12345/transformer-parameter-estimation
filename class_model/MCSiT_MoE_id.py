...
from task_moe_id.layer import TaskMoE
...
class PositionwiseFeedForward(nn.Module):
    def __init__(self, d_in, d_hid, dropout=0.1):
        ...
        self.linear1 = MoE_layer(hidden_size=d_in, expert=self.w_1)
        self.linear2 = MoE_layer(hidden_size=d_hid, expert=self.w_2)

    def forward(self, x):
        residual = x
        x = rearrange(x, 'b (h n) c -> b h n c', h=4)
        x1, gate_decision = self.linear1(x[:, 0, :, :],0)
        x1=self.layer_norm1(x1)
        x1, _ = self.linear2(self.dropout(F.relu(x1)),0,gate_decision=gate_decision)
        x2, gate_decision = self.linear1(x[:, 1, :, :],1)
        x2 = self.layer_norm2(x2)
        x2, _ = self.linear2(self.dropout(F.relu(x2)),1,gate_decision=gate_decision)
        x3, gate_decision = self.linear1(x[:, 2, :, :],2)
        x3 = self.layer_norm3(x3)
        x3, _ = self.linear2(self.dropout(F.relu(x3)),2,gate_decision=gate_decision)
        x4, gate_decision = self.linear1(x[:, 3, :, :],3)
        x4 = self.layer_norm4(x4)
        x4, _ = self.linear2(self.dropout(F.relu(x4)),3,gate_decision=gate_decision)
