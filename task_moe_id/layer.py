...
class TaskMoE(torch.nn.Module):
    ...
    def forward(self, hidden_states, task_id, gate_decision=None, **kwargs):
        """ MoE forward
        Arguments:
        ...
        if  gate_decision is not None:
            ...
            input=torch.nn.Embedding(4, 128)(torch.tensor(task_id).cpu())
            top_indices, gates = self.gate(input.to('cuda:0'), **kwargs)
            ##2×128×20 #2×128
